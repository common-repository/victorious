...
class VIC_TableTeams extends WP_List_Table
{
    ...
    function column_default( $item, $column_name ) 
    {
        ...
        { 
            ...
            case 'salary':
                return '<input type="text" name="salary['.esc_html($item['teamID']).']" value="'.number_format($item[ $column_name ]).'" onkeyup="this.value = accounting.formatNumber(this.value)" />';
            case 'edit':
                $sport_param = !empty($_GET['sport_id']) ? '&sport_id='.sanitize_text_field($_GET['sport_id']) : '';
                return sprintf('<a href="?page=%s'.$sport_param.'&action=%s&id=%s">Edit</a>', 'add-teams','edit',esc_html($item['teamID']));
            default:
    ...
    function prepare_items($keyword = null) 
    {
        ...
        list($total_items, $aPools) = self::$teams->getTeamsByFilter($sport_id, $aCond, 'teamID DESC', ($this->get_pagenum() - 1) * $item_per_page, $item_per_page);
