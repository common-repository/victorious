...
jQuery.league =
{
                ...
                for(var i = 0;i < aLeagues.length; i++)
                {
                    ...
                    html +=
                        '<tr>\n\
                            ...
                            <td>' + VIC_FormatMoney(aLeague.entry_fee) + ' / ' + VIC_FormatMoney(aLeague.prizes) + '</td>\n\
    ...
    dlgLobbyInfo: function(leagueID)
    {
        ...
        var htmlGeneral = 
            ...
            <br><b>' + wpfs['Entry Fee'] + ': </b>' + formatMoneyaLeague(entry_fee) + '\n\
            <br><b>' + wpfs['Prizes'] + ': </b>' + VIC_FormatMoney(aLeague.prizes) + '\n\
    ...
    loadLiveEntries: function()
    {
        var data = {
            action: 'loadLiveEntries',
        };
        jQuery.post(ajaxurl, data, function(result) {
            jQuery('#tableLiveEntriesContent').empty().append(result);
        })
    },
    ...
    liveEntriesResult: function(poolID, leagueID, entry_number, is_live_draft)
    {
        ...
        jQuery.post(ajaxurl, data, function(result) {
            jQuery.playerdraft.loadFixtureScores(leagueID);
