<?php
class VIC_Statistic extends VIC_Model
{
    ...
    public function eventStatistic($leagueID)
    {
        ...
        if($aLeague != null)
        {
            if(!empty($aLeague['pick']))
            {
                ...
                {
                    $user = $fv->get_user_info($pick['userID']);
                    if($user != null)
                    {
            ...
            if(!empty($aLeague['best_inviter']))
            {
                $best_inviter = array();
                foreach($aLeague['best_inviter'] as $user_id)
                {
                    $user = $fv->get_user_info($user_id);
                    $best_inviter[] = $user->user_login;
                }
                $aLeague['best_inviter'] = implode(', ', $best_inviter);
            }
        }
        return $aLeague;
    }
